...
@pytest.fixture
def domain_event() -> NotificationEmailCreatedDomainEvent:
    return NotificationEmailCreatedDomainEvent(
...
class TestNotificationCreatedDomainEvent:
    def test_should_have_valid_event_id(
        self, domain_event: NotificationEmailCreatedDomainEvent
    ) -> None:
    ...
    def test_should_have_event_name(
        ...
        assert (
            domain_event.get_event_name() == "landbot.event.notification.email.created"
        )
